...
module API::V2::Queries
  class ActivityFilter
    ...
    def filter_by_date(scoped, range = 'created', from = nil, to = nil)
      updated_scope = from ? scoped.where("activities.#{range}_at >= ?", Time.at(from.to_i)) : scoped
      to ? updated_scope.where("activities.#{range}_at <= ?", Time.at(to.to_i)) : updated_scope
    end
